...
sql = """insert into landcrawler.realestate_article(clct_dt, complexNo, articleNo, tradeTypeName, floorInfo, dealOrWarrantPrc, areaName, area1, area2, buildingName)
         values (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"""

today = str(date.today().strftime('%Y%m%d'))

def printData (complexNo, jsonList) :
    lineCnt=1
...
# complexNo=104917       # 마포래미안푸르지오 (마포구)
# complexNo=111515        # 헬리오시티 (송파구)
# complexNo=27355       # 용산이편한세상 (용산구)
# complexNo=102329        # 용산더프라임 (용산구)
...
conn = pymysql.connect(host='127.0.0.1', user='landcrawler', password='landcrawler',
                db='landcrawler', charset='utf8')
curs = conn.cursor()    
curs.execute("select complexNo from landcrawler.realestate_apt_complex")

complexList = curs.fetchall()
...
for complex in complexList:
    ...
    for tradeType in tradeTypeList:        
        ...
        while bool(moreData) :                
                ...
                # print(url)
                printData(complexNo, articleList)
